...
@testset "ONSAS.BoundaryConditions.NormalTension" begin

    # Generic labeled global load boundary condition
    dofs_toapply_bc = [:u]
    load_fact_generic(t) = t^2
    generic_values = t -> load_fact_generic(t)
    generic_bc_label = :normal_tension
    generic_bc = NormalTension(; dofs=dofs_toapply_bc, values=generic_values, name=generic_bc_label)

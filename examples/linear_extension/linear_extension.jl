...
function numerical_solution(sol::AbstractSolution,
	p1::Point{dim}, p2::Point{dim}, e::ONSAS.AbstractElement{dim}) where {dim}
	s = ONSAS.structure(analysis(sol))
	## Displacements
...
function analytic_solution(sol::AbstractSolution, p1::Point{dim}, p2::Point{dim}, e::AbstractElement{dim}) where {dim}
	(; E, ν, λ, G, tension) = parameters()
	sa = analysis(sol)
	## Displacements
	"Compute displacements νmeric solution ui, uj and uk for analytic validation."
	function u_ijk_analytic(
		λv::Vector{<:Real},

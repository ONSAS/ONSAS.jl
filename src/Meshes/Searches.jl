...
"Return indexes checking if  a `Vector` of `Point`s is inside a `Vector` of 
`Element`s with a `Serial` searching algorithm. Two `Vector` of `Int` are returned,
...
"Return indexes checking if  a `Vector` of `Point`s is inside a `Vector` of 
`Element`s with a `Threaded` searching algorithm. Two `Vector` of `Int` are returned,
the first one contains the indexes of the points inside the mesh, the second one
contains the indexes of the elements containing the points."
function _evaluate_points_in_elements(elements::Vector{E}, vec_points::Vector{P},
...
"Return the ordered indexes following the `Vector` of `Point`s"
...
"Return a box approximation of a `Vector` of `Node`s transforming them to LazySets `Singleton`s."
...
"Return indexes checking if  a `Vector` of `Point`s is inside an `AbstractMesh`
with a `Partition`ed searching algorithm. Two `Vector` of `Int` are returned,
the first one contains the indexes of the points inside the mesh, the second one
contains the indexes of the elements containing the points."

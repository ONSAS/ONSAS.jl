...
export MshFile, connectivity, material_label, entity_label, bc_label, physical_indexes
...
struct MshFile{dim,T,S,I1<:Integer,I2<:Integer}
    ...
    function MshFile(filename::String, vec_nodes::Vector{Node{dim,T}},
        connectivity::Vector{Vector{I1}}, physical_indexes::Vector{I2},
        material_labels::Vector{S}, entities_labels::Vector{S}, bc_labels::Vector{S}
    ) where {dim,T,S,I1<:Integer,I2<:Integer}

        @assert length(physical_indexes) == length(connectivity) "The number of physical indexes = $(length(physical_indexes))
        ...
        return new{dim,T,S,I1,I2}(filename, vec_nodes, connectivity, physical_indexes, material_labels, entities_labels, bc_labels)
    end
end
...
"Returns physical_indexes defined in the `MshFile``mfile`."
physical_indexes(mfile::MshFile) = mfile.physical_indexes
...
material_label(mfile::MshFile, entity_index::Integer) = material_label(mfile)[physical_indexes(mfile)[entity_index]]
...
entity_label(mfile::MshFile, entity_index::Integer) = entity_label(mfile)[physical_indexes(mfile)[entity_index]]
...
bc_label(mfile::MshFile, entity_index::Integer) = bc_label(mfile)[physical_indexes(mfile)[entity_index]]
